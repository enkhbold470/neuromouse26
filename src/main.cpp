...
// Tests: DRV8833 wake, both motors FWD/REV at varying speeds, encoder tick counts, direction
#include <Arduino.h>
#include "PinConfig.h"
...
// Run one motor at a given PWM for ms, then brake+coast. Print ticks.
...
void setup() {
    ...
    runTest(leftMotor, leftEnc, "LEFT  FWD  30%", +307, 500);
    delay(300);
    runTest(leftMotor, leftEnc, "LEFT  FWD  60%", +614, 500);
    delay(300);
    runTest(leftMotor, leftEnc, "LEFT  FWD 100%", +1023, 500);
    delay(300);
    runTest(leftMotor, leftEnc, "LEFT  REV  50%", -512, 500);
    delay(500);

    // ── RIGHT motor tests ─────────────────────────────────────────────
    Serial.println("[TEST] === RIGHT MOTOR ===");
    runTest(rightMotor, rightEnc, "RIGHT FWD  30%", +307, 500);
    delay(300);
    runTest(rightMotor, rightEnc, "RIGHT FWD  60%", +614, 500);
    delay(300);
    runTest(rightMotor, rightEnc, "RIGHT FWD 100%", +1023, 500);
    delay(300);
    runTest(rightMotor, rightEnc, "RIGHT REV  50%", -512, 500);
    delay(500);
